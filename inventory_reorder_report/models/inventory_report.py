...
class inventory_report(models.AbstractModel):
    ...
    _description = 'Inventory Report By Warehouse'
        ...
        self._cr.execute('''
                            SELECT pp.id AS product_id,
                                sum((
                                    ...
                                    WHEN sm.location_id in %s AND sourcel.usage !='inventory' AND sm.location_id = swo.location_id AND  destl.scrap_location IS TRUE
                                    THEN -(sm.product_qty * pu.factor / pu2.factor)
                                    ELSE 0.0 
                                    END
                                )) AS product_qty_out,
                                ...
                                MIN((
                                    CASE WHEN swo.location_id in %s AND (sm.location_id = swo.location_id OR sm.location_dest_id = swo.location_id)
                            ...
                            ''',(tuple(locations),tuple(locations),tuple(locations),tuple(locations),tuple(locations),tuple(locations),tuple(locations),tuple(locations),tuple(locations),start_date, end_date))

...
class SaleSampleGive(models.Model):
    ...
    location_dest_id = fields.Many2one(
        ...
        track_visibility='always', readonly=True, Store=True)
    sample_remark = fields.Text(string="Remark")
    accept_person = fields.Char(string='Accept Person')
    sale_person = fields.Many2one('res.users', string='Sale By', track_visibility='always',
    ...
    branch_id = fields.Many2one('res.branch', string='Branch', track_visibility='always',
                                default=lambda self: self.env.user.branch_id)

    product_line = fields.One2many('sample.give.line', 'give_id', string='Product Lines',
                                   states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True,
                                   auto_join=True)

    picking_ids = fields.One2many('stock.picking', 'sample_id', string='Pickings')
    ...
    def _onchange_branch(self):
        if self.branch_id:
            ...
            if self.branch_id:
                print(self.branch_id.name)
                self.product_line.product_id = product.ids[0]
                print(len(self.product_line.product_id))
            else:
                self.product_id = False
            return {'domain': {'product_line.product_id': [('id', '=', product.ids)]}}
        else:

            return {'domain': {'product_line.product_id': []}}
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('sample.give') or _('New')
        res = super(SaleSampleGive, self).create(vals)
        return res

    @api.onchange('sample_give_date')
    def onchange_sample_give_date(self):
        self.return_date = self.sample_give_date + relativedelta(months=1)
    ...
    def action_confirmed(self):
        ...
        for picking_type, damage_lines in groupby(lines, key=lambda l: l.picking_type_id):
            ...
            picking_values.update({
                ...
                'sample_give_date': self.sample_give_date,
                'return_date': self.return_date,
                'branch_id': self.branch_id.id,
                'sample_remark': self.sample_remark,
...
class SaleSampleGiveLine(models.Model):
    _name = 'sample.give.line'
    _description = 'Sample Give Product Line'

    give_id = fields.Many2one('sample.give', string='Reference', required=True, ondelete='cascade', index=True,
                              copy=False)
    ...
    def compute_inventory_quantity(self):
        for line in self:
            stock_quant = self.env['stock.quant'].search(
                [('product_id', '=', line.product_id.id), ('location_id', '=', line.give_id.location_id.id)], limit=1)

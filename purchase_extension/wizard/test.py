...
class POListWizard(models.TransientModel):
    ...
    def _purchse_order_within_date(self, from_date, to_date, supplier_id):
        if supplier_id:
            self._cr.execute(
                '''SELECT rp.id as rpid, rp.vendor_code as vendor, rp.name as vendor_name ,po.name as o_reference,
                    crp.name as contact,pt.name as Item,pol.name as Description,am.name as bill_name,
                    ...
                    GROUP BY rp.id, rp.name,crp.name,cur.symbol,cur.id,pt.name,pol.name,pol.product_qty,
                        po.date_order,pu.name,pol.price_subtotal,po.date_approve,po.name,
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write(y_offset, 7, _('Item'), format1)

        sheet.write(y_offset, 8, _('Description'), format1)
        ...
        if order_ids:
            ...
            for line in order_ids:
                ...
                query = """SELECT rate FROM res_currency_rate WHERE currency_id=""" + (
                    str(line['currency_id'])) + """ ORDER BY create_date DESC LIMIT 1""";
                ...
                if vendor_id == line['rpid']:
                    ...
                    if line['currency'] == '$':
                        subtotal_usd += (line['amount'] * rate)
                    elif line['currency'] == '¥':
                        subtotal_yuan += (line['amount'] * rate)
                    else:
                        subtotal += line['amount']
                    if line['bill_amount']:
                        if line['currency'] == '$':
                            usd_total += (line['bill_amount'] * rate)
                        elif line['currency'] == '¥':
                            yuan_total += (line['bill_amount'] * rate)
                        else:
                    ...
                    sheet.write(y_offset, 13, subtotal, format4)
                    sheet.write(y_offset, 15, bill_amount, format4)
                    y_offset += 2
                    quantity = 0
                    qty_received = 0
                    qty_invoiced = 0
                ...
                sheet.write(y_offset, 7, line['item'], format3)

                sheet.write(y_offset, 8, line['description'], format3)
                ...
                if line['currency'] == '$':
                    usd_total += (line['amount'] * rate)
                elif line['currency'] == '¥':
                    yuan_total += (line['amount'] * rate)
                else:
                    total += line['amount']
                if line['bill_amount']:
                    if line['currency'] == 'K':
            ...
            sheet.write(y_offset, 10, qty_received, format4_qty)
            sheet.write(y_offset, 11, qty_invoiced, format4_qty)
            sheet.write(y_offset, 14, subtotal_usd, format4)
            sheet.write(y_offset, 15, subtotal_yuan, format4)
            sheet.write(y_offset, 16, subtotal, format4)
            sheet.write(y_offset, 19, bill_amount, format4)
            y_offset += 2
            col = 0
            while col < 21:
                sheet.write(y_offset, col, '', format4)
            ...
            sheet.write(y_offset, 16, total, format5)

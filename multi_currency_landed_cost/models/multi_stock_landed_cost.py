...
class MultiLandedCost(models.Model):
    _name = 'multi.stock.landed.cost'
    _description = 'Multi Stock Landed Cost'
    ...
    cost_lines = fields.One2many(
        'multi.stock.landed.cost.lines', 'cost_id', 'Multi Cost Lines',
        copy=True, states={'done': [('readonly', True)]})
    valuation_adjustment_lines = fields.One2many(
        'multi.stock.valuation.adjustment.lines', 'cost_id', 'Valuation Adjustments',
    ...
    vendor_bill_id = fields.Many2one('account.move', 'Vendor Bill', copy=False, domain=[('type', '=', 'in_invoice')])
    company_id = fields.Many2one('res.company', string="Company",
        related='account_journal_id.company_id')
    stock_valuation_layer_ids = fields.One2many('stock.valuation.layer', 'multi_stock_landed_cost_id')
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('multi.stock.landed.cost')
        return super(MultiLandedCost, self).create(vals)

    def unlink(self):
        self.button_cancel()
        return super(MultiLandedCost, self).unlink()

    def _track_subtype(self, init_values):
        if 'state' in init_values and self.state == 'done':
            return self.env.ref('multi_currency_landed_cost.mt_multi_stock_landed_cost_open')
        return super(MultiLandedCost, self)._track_subtype(init_values)
    ...
    def action_draft(self):
        way = self.filtered(lambda s: s.state in ['cancel'])
        return way.write({'state': 'draft'})
            ...
            for line in cost.valuation_adjustment_lines.filtered(lambda line: line.move_id):
                ...
                if not cost.company_id.currency_id.is_zero(cost_to_add):
                    valuation_layer = self.env['stock.valuation.layer'].create({
                        ...
                        'multi_stock_landed_cost_id': cost.id,
    ...
    def compute_landed_cost(self):
        AdjustementLines = self.env['multi.stock.valuation.adjustment.lines']
        ...
        for cost in self.filtered(lambda cost: cost.picking_ids):
            ...
            for val_line_values in all_val_line_values:
                for cost_line in cost.cost_lines:
                    ...
                    self.env['multi.stock.valuation.adjustment.lines'].create(val_line_values)
...
class LandedCostLine(models.Model):
    _name = 'multi.stock.landed.cost.lines'
    _description = 'Multi Stock Landed Cost Line'

    name = fields.Char('Description')
    cost_id = fields.Many2one(
        'multi.stock.landed.cost', 'Multi Landed Cost',
    ...
    rate = fields.Float(digits=(0, 12), default=1.0, string='Rate', related='currency_id.rate',
                        help='The rate of the currency to the currency of rate 1')
    amount_currency = fields.Monetary(currency_field="currency_id", string='Amount in Currency', store=True, copy=True,
        ...
        for rec in self:
            if rec.amount_currency:
                ...
                if rec.currency_id != company.currency_id:
                    rec.price_unit = rec.currency_id._convert(rec.amount_currency, company.currency_id, company, date)
                else:
                    rec.price_unit = rec.amount_currency
    ...
    def onchange_product_id(self):
        ...
        if self.product_id.product_tmpl_id.product_description:
            aa = self.product_id.product_tmpl_id.product_description
            self.name = self.product_id.name + aa or ''
        else:
            bb = ''
            self.name = self.product_id.name  or ''
        self.split_method = self.split_method or 'equal'
        self.price_unit = self.product_id.standard_price or 0.0
        self.amount_currency = self.price_unit
        accounts_data = self.product_id.product_tmpl_id.get_product_accounts()
        self.account_id = accounts_data['stock_input']


class AdjustmentLines(models.Model):
    _name = 'multi.stock.valuation.adjustment.lines'
    _description = 'Multi Valuation Adjustment Lines'
    ...
    cost_line_id = fields.Many2one(
        'multi.stock.landed.cost.lines', 'Multi Cost Line', readonly=True)
